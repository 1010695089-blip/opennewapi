...
import { NotFoundError, OpenHingeError, toOpenAIError } from '../../utils/errors.js';
...
async function handleChat(request: FastifyRequest<{ Params?: { slug?: string }; Body: ChatBody }>, reply: FastifyReply): Promise<void> {
  ...
  if (slugParam) {
    ...
    // New branded header with legacy header fallback.
  ...
  if (body.stream) {
    ...
    try {
      ...
      reply.raw.write(`data: ${JSON.stringify({ error: toOpenAIError(err) })}\n\n`);
    } finally {
      reply.raw.end();
