...
export async function providerAdminRoutes(app: FastifyInstance, config: Config): Promise<void> {
  ...
  app.get<{ Params: { id: string } }>('/admin/providers/:id/models', async (request, reply) => {
    const row = getDb().prepare('SELECT * FROM providers WHERE id = ?').get(request.params.id) as any;
    ...
    try {
      ...
      try {
        providerConfig.credentials = JSON.parse(decrypt(row.credentials, config.encryption.key));
      } catch {
        providerConfig.credentials = JSON.parse(row.credentials || '{}');
      }
      ...
      switch (row.type) {
        ...
        case 'gemini': provider = new GeminiProvider(providerConfig); break;
        case 'ollama': provider = new OllamaProvider(providerConfig); break;
        default: return reply.code(400).send({ error: `Unknown type: ${row.type}` });
      }
  ...
  const geminiBaseMetadata = {
    ideType: 'IDE_UNSPECIFIED',
    pluginType: 'GEMINI',
    platform: 'PLATFORM_UNSPECIFIED',
  };

  function getConfiguredGeminiProjectId(): string {
    const projectId = process.env.GOOGLE_CLOUD_PROJECT
      || process.env.GOOGLE_CLOUD_PROJECT_ID
      || process.env.GEMINI_PROJECT_ID
      || '';
    if (/^\d+$/.test(projectId)) {
      throw new Error('GOOGLE_CLOUD_PROJECT must be the string project ID, not the numeric project number.');
    }
    return projectId;
  }

  function extractGeminiProjectId(project: unknown): string {
    if (typeof project === 'string') return project;
    if (project && typeof project === 'object') {
      const p = project as { id?: unknown; name?: unknown };
      if (typeof p.id === 'string') return p.id;
      if (typeof p.name === 'string') return p.name.split('/').pop() || '';
    }
    return '';
  }

  function formatGeminiSetupError(data: any): string {
    const validationTier = data?.ineligibleTiers?.find((t: any) => t.validationUrl || t.reasonMessage);
    if (validationTier) {
      const reason = validationTier.reasonMessage || validationTier.validationErrorMessage || validationTier.reasonCode;
      const link = validationTier.validationUrl ? ` ${validationTier.validationUrl}` : '';
      return `Google account is not eligible for Gemini Code Assist: ${reason || 'unknown reason'}.${link}`;
    }
    const message = data?.error?.message || data?.message;
    if (message) return message;
    return 'Google account requires a Google Cloud project ID. Set GOOGLE_CLOUD_PROJECT to the string project ID in .env, then restart OpenHinge.';
  }

  async function codeAssistPost(accessToken: string, method: string, body: Record<string, unknown>) {
    const res = await fetch(`${codeAssistBaseUrl}:${method}`, {
      method: 'POST',
      headers: {
  ...
  async function codeAssistGet(accessToken: string, path: string) {
    const res = await fetch(`${codeAssistBaseUrl}/${path}`, {
      headers: {
        'Authorization': `Bearer ${accessToken}`,
        'Content-Type': 'application/json',
        'User-Agent': 'google-cloud-sdk vscode_cloudshelleditor/0.1',
  ...
  async function discoverGeminiProjectId(accessToken: string): Promise<string> {
    let projectId = getConfiguredGeminiProjectId();
    const metadata = projectId
      ? { ...geminiBaseMetadata, duetProject: projectId }
      : geminiBaseMetadata;
    ...
    console.log('loadCodeAssist status:', loadRes.status, 'response:', JSON.stringify(loadRes.data));

    if (!loadRes.ok) {
      throw new Error(formatGeminiSetupError(loadRes.data));
    }

    projectId = extractGeminiProjectId(loadRes.data.cloudaicompanionProject) || projectId;
    if (projectId) return projectId;

    if (loadRes.data.currentTier) {
      throw new Error(formatGeminiSetupError(loadRes.data));
    }

    const allowedTiers = Array.isArray(loadRes.data.allowedTiers) ? loadRes.data.allowedTiers : [];
    const tier = allowedTiers.find((t: any) => t.isDefault) || allowedTiers[0];
    if (!tier?.id) {
      throw new Error(formatGeminiSetupError(loadRes.data));
    }

    const onboardProject = tier.id === 'free-tier' ? undefined : getConfiguredGeminiProjectId() || undefined;
    const onboardMetadata = onboardProject
      ? { ...geminiBaseMetadata, duetProject: onboardProject }
      : geminiBaseMetadata;
    const onboardRes = await codeAssistPost(accessToken, 'onboardUser', {
      tierId: tier.id,
      cloudaicompanionProject: onboardProject,
      metadata: onboardMetadata,
    });
    console.log('onboardUser status:', onboardRes.status, 'response:', JSON.stringify(onboardRes.data));

    if (!onboardRes.ok) {
      throw new Error(formatGeminiSetupError(onboardRes.data));
    }

    let operation = onboardRes.data;
    for (let i = 0; i < 12 && operation.name && !operation.done; i++) {
      await new Promise(r => setTimeout(r, 5000));
      const pollRes = await codeAssistGet(accessToken, operation.name);
      console.log(`onboardUser poll ${i}:`, JSON.stringify(pollRes.data));
      if (!pollRes.ok) {
        throw new Error(formatGeminiSetupError(pollRes.data));
      }
      operation = pollRes.data;
    }

    projectId = extractGeminiProjectId(operation.response?.cloudaicompanionProject) || onboardProject || '';
    if (projectId) return projectId;
      ...
      if (reqUrl.pathname === oauthCfg.callbackPath && req.method === 'GET') {
        ...
        (async () => {
          ...
          if (oauthCfg.providerType === 'gemini') {
            const projectId = await discoverGeminiProjectId(accessToken);
