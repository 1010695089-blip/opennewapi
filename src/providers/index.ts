...
export async function chatWithFallback(
  ...
  throw new ProviderError('all', `All providers failed — ${summary}`, inferProviderFailureStatus(attempts));
}
...
function inferProviderFailureStatus(attempts: FallbackAttempt[]): number {
  if (attempts.length === 0) return 502;
  if (attempts.every(a => a.error.includes('Provider [') && a.error.includes('429:'))) return 429;
  if (attempts.every(a => a.error.includes('Provider [') && /(?:401|403):/.test(a.error))) return 502;
  return 502;
}
