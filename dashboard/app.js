const OS = (() => {
  ...
  // ===== Navigation =====
  const titles = {
    dashboard: 'Command Center', providers: 'Upstreams', souls: 'Profiles',
    keys: 'API Keys', usage: 'Usage', logs: 'Traffic Ledger',
    cloudflare: 'Edge Tunnel', settings: 'Settings', docs: 'Field Manual',
  };
  ...
  function navigate(page, pushHash = true) {
    ...
    Promise.resolve(loaders[page]?.()).finally(() => applyLanguage());
  }
  ...
  function applyLanguage() {
    window.AstraI18n?.apply(document.body);
  }

  function setLanguage(lang) {
    window.AstraI18n?.setLanguage(lang);
  }
  ...
  loaders.dashboard = async () => {
    ...
    el.innerHTML = `
      <div class="stats-grid">
        ${statCard('Upstreams', status.providers, 'Active connections')}
        ${statCard('Profiles', status.souls, 'AI endpoints')}
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
        <div class="card">
          ...
          ${providers.length ? `<div class="table-wrapper" style="border:none">
            <table><thead><tr><th>Upstream</th><th>Type</th><th>Status</th></tr></thead>
          ...
          </div>` : '<p class="text-muted text-sm" style="padding:16px">No upstreams configured</p>'}
        </div>
        ...
        <div class="card">
          ...
          ${souls.length ? `<div class="table-wrapper" style="border:none">
            <table><thead><tr><th>Profile</th><th>Slug</th><th>Model</th></tr></thead>
          ...
          </div>` : '<p class="text-muted text-sm" style="padding:16px">No profiles created</p>'}
        </div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-top:16px">
        <div class="card">
          ...
          ${logs.data?.length ? `<div class="table-wrapper" style="border:none">
            <table><thead><tr><th>Profile</th><th>Upstream</th><th>Tokens</th><th>Latency</th><th>Time</th></tr></thead>
  ...
  loaders.providers = async () => {
    ...
    el.innerHTML = `
      <div class="section-header">
        <div><h2>LLM Upstreams</h2><p>${allData.length} upstream${allData.length !== 1 ? 's' : ''} configured</p></div>
      ...
      <div class="log-toolbar">
        <div class="log-search">
          ...
          <input type="text" placeholder="Search upstreams..." value="${h(_provFilter.search)}" onkeydown="if(event.key==='Enter')OS.provFilter('search',this.value)" onblur="OS.provFilter('search',this.value)">
        </div>
      ...
      </table></div>` : emptyState('No upstreams found', hasFilters ? 'Try adjusting your filters.' : 'Add an upstream to start routing AI requests.')}
  ...
  loaders.souls = async () => {
    ...
    el.innerHTML = `
      <div class="section-header">
        <div><h2>Profiles</h2><p>Routable AI behaviors with prompts and dedicated endpoints</p></div>
      ...
      ${souls?.length ? `<div class="table-wrapper"><table>
        <thead><tr><th>Name</th><th>Endpoint</th><th>Upstream</th><th>Model</th><th>Status</th><th></th></tr></thead>
      ...
      </table></div>` : emptyState('No profiles configured', 'Create a profile to define an AI endpoint with a system prompt.')}
    `;
  };
  ...
  loaders.keys = async () => {
    ...
    const soulBadges = (k) => {
      ...
      return '<span class="badge badge-success">All profiles</span>';
    };
    ...
    el.innerHTML = `
      <div class="section-header">
        <div><h2>API Keys</h2><p>Issue and revoke access tokens for consumers</p></div>
      ...
      ${data?.length ? `<div class="table-wrapper"><table>
        <thead><tr><th>Name</th><th>Prefix</th><th>Format</th><th>Profiles</th><th>Rate Limit</th><th>Requests</th><th>Last Used</th><th></th></tr></thead>
  ...
  loaders.logs = async () => {
    ...
    el.innerHTML = `
      <div class="section-header"><div><h2>Traffic Ledger</h2><p>${num(total)} total request${total !== 1 ? 's' : ''}</p></div></div>

      <div class="log-toolbar">
        <div class="log-search">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <input type="text" placeholder="Search by model, profile, upstream, error..." value="${h(_logFilters.search)}" onkeydown="if(event.key==='Enter')OS.logFilter('search',this.value)">
        </div>
        <div class="log-divider"></div>
        <div class="log-filters">
          <select onchange="OS.logFilter('soul_id',this.value)">
            <option value="">All Profiles</option>${soulOpts}
          </select>
          <select onchange="OS.logFilter('provider_id',this.value)">
            <option value="">All Upstreams</option>${provOpts}
          </select>
      ...
      ${data?.length ? `<div class="table-wrapper"><table class="traffic-table">
        <colgroup>
          <col class="traffic-col-time">
          <col class="traffic-col-key">
          <col class="traffic-col-profile">
          <col class="traffic-col-upstream">
          <col class="traffic-col-model">
        ...
        <tbody>${data.map(l => `<tr class="log-row" onclick="OS.logDetail('${h(l.request_id || '')}')">
          ...
          <td><span class="text-mono text-sm truncate traffic-model">${h(l.model)}</span></td>
  ...
  loaders.cloudflare = async () => {
    ...
    el.innerHTML = `
      <div class="section-header"><div><h2>Edge Tunnel</h2><p>Cloudflare tunnel and DNS controls for external access</p></div></div>
    ...
    el.innerHTML = `
      ...
      <div class="card" style="max-width:600px">
        <form onsubmit="OS.saveSettings(event)">
          <div class="form-group">
            ...
            <input name="name" value="${h(g.name || 'Astra Relay')}">
          </div>
          <div class="form-group">
      ...
      <div class="card mt-4" style="max-width:600px">
        ...
        <div style="display:flex;flex-direction:column;gap:8px">
          ...
          <div><span class="text-muted text-sm">Profile-specific</span><div class="code-block">POST /v1/souls/{slug}/chat/completions</div></div>
    ...
    el.innerHTML = `
      <div class="docs">
        <div class="docs-nav">
          ...
          <a href="#doc-providers" class="docs-link" onclick="OS.scrollDoc(event,'doc-providers')">Upstreams</a>
          <a href="#doc-auth" class="docs-link" onclick="OS.scrollDoc(event,'doc-auth')">Authentication</a>
          <a href="#doc-souls" class="docs-link" onclick="OS.scrollDoc(event,'doc-souls')">Profiles</a>
          ...
          <a href="#doc-logs" class="docs-link" onclick="OS.scrollDoc(event,'doc-logs')">Traffic Ledger</a>
          <a href="#doc-cloudflare" class="docs-link" onclick="OS.scrollDoc(event,'doc-cloudflare')">Edge Tunnel</a>
        ...
        <div class="docs-content">
          ...
          <section id="doc-overview">
            <h2>Overview</h2>
            <p>Astra Relay is a private AI routing console that unifies multiple LLM upstreams behind a single OpenAI-compatible API. Run it on your own machine, use your own subscriptions and API keys, and control who can access what.</p>
            <div class="doc-card">
              <h4>Key Concepts</h4>
              <dl class="doc-dl">
                <dt>Upstreams</dt><dd>Connections to LLM services. Supports Claude, OpenAI, Gemini, and Ollama. Each can authenticate via OAuth or API key. Multiple upstreams of the same type are supported.</dd>
                <dt>Profiles</dt><dd>Routable AI behaviors with system prompts. Each profile has its own endpoint, upstream, model, and behavior rules. Examples: Translator, Blog Writer, Code Reviewer.</dd>
                <dt>API Keys</dt><dd>Access tokens (<code>ohk_</code> prefix) for consumers. Each key has rate limits, optional profile binding, and budget controls.</dd>
                <dt>Fallback Chain</dt><dd>If an upstream is down, requests automatically route to the next healthy upstream by priority.</dd>
              </dl>
            </div>
          </section>
          ...
          <section id="doc-quickstart">
            ...
            <div class="doc-step">
              ...
              <div>
                <h4>Add an upstream</h4>
                <p>Go to <a href="#" onclick="OS.navigate('providers');return false" style="color:var(--primary)">Upstreams</a> and click "Add Upstream". Choose your upstream type and either log in with OAuth or enter an API key.</p>
              </div>
            </div>
            ...
            <div class="doc-step">
              ...
              <div>
                ...
                <p>Go to <a href="#" onclick="OS.navigate('souls');return false" style="color:var(--primary)">Profiles</a> and click "Add Profile". Pick an upstream and model, write a system prompt that defines how the AI should behave, and save.</p>
            ...
            <div class="doc-step">
              ...
              <div>
                ...
                <p>Go to <a href="#" onclick="OS.navigate('keys');return false" style="color:var(--primary)">API Keys</a> and click "Create Key". Select which profiles the key can access, set a rate limit, and save. Copy the key immediately — it won't be shown again.</p>
              </div>
            </div>
  ...
  -d '{"messages":[{"role":"user","content":"Translate to German: Hello"}]}'</div>
          ...
          <section id="doc-install">
            ...
            <div class="doc-card">
              <h4>Requirements</h4>
              <p>Node.js 18+ and npm. No external databases — Astra Relay uses embedded SQLite.</p>
            </div>
...
npm run build
npm start</div>
              <p class="form-hint" style="margin-top:8px">On first run, Astra Relay generates your config automatically. Open <code>http://localhost:3700</code> and set a password to get started.</p>
            </div>
            <div class="doc-card mt-4">
              <h4>Development</h4>
    ...
    v
Astra Relay Core (Fastify + SQLite)
    |
    |  1. Authenticate API key (ohk_...)
    |  2. Rate limit + budget check
    |  3. Resolve profile -> system prompt + model
    |  4. Pick upstream (priority + health + fallback)
...
LLM Upstreams
    ...
    |-- Ollama (local, no auth)</div>
          ...
          <section id="doc-providers">
            <h2>Upstreams</h2>
            <p>Upstreams are connections to LLM services. You can add multiple upstreams of the same type (e.g., two Claude accounts).</p>
            ...
            <div class="doc-card mt-4">
              ...
              <p class="form-hint">Models: gemini-3-pro-preview, gemini-3-flash-preview, gemini-2.5-pro, etc.</p>
            </div>
            <div class="doc-card mt-4">
              <h4>Ollama (Local)</h4>
            ...
            <div class="doc-card mt-4">
              ...
              <p>Each upstream has a <strong>priority</strong> (higher = tried first). When an upstream is down or returns an error, Astra Relay automatically tries the next healthy upstream. Enable/disable upstreams without deleting them.</p>
            </div>
            <div class="doc-card mt-4">
          ...
          <section id="doc-auth">
            ...
            <p>Astra Relay supports two ways to authenticate with each upstream:</p>
            <div class="doc-card">
          ...
          <section id="doc-souls">
            <h2>Profiles</h2>
            <p>Profiles are AI behaviors that define how the model responds. Each profile gets its own dedicated endpoint.</p>
            <div class="doc-card">
              <dl class="doc-dl">
                <dt>Upstream & Model</dt><dd>Which LLM handles this profile's requests</dd>
            ...
            <div class="doc-card mt-4">
              <h4>Profile Resolution</h4>
              <p>The profile for a request is determined in this order:</p>
              <ol class="doc-ol">
                <li>URL path: <code>/v1/souls/:slug/chat/completions</code></li>
                <li>If the API key is bound to exactly one profile, it auto-resolves</li>
                <li><code>X-Astra-Profile</code> header with the profile slug</li>
              </ol>
            </div>
          </section>
          ...
          <section id="doc-keys">
            ...
            <div class="doc-card">
              <dl class="doc-dl">
                ...
                <dt>Profile Binding</dt><dd>Restrict a key to specific profiles (many-to-many), or allow access to all</dd>
  ...
  "stream": false
              ...
              <p class="form-hint" style="margin-top:6px">If no model is specified, the profile default model is used. If no profile is specified, the first available upstream default model is used.</p>
            </div>
            <div class="doc-card mt-4">
              <h4>POST /v1/souls/:slug/chat/completions</h4>
              <p>Routes to a specific profile. The profile's system prompt is automatically prepended to the messages. Upstream and model are determined by the profile configuration.</p>
            </div>
            <div class="doc-card mt-4">
              <h4>GET /v1/models</h4>
              <p>Lists all available models across all enabled upstreams. Returns OpenAI-compatible format.</p>
            </div>
            <div class="doc-card mt-4">
              <h4>GET /health</h4>
  ...
  -d '{"messages":[{"role":"user","content":"Hello"}],"stream":true}'</div>
          ...
          <section id="doc-logs">
            ...
            <p>Every API request is logged with full details. View logs in the <a href="#" onclick="OS.navigate('logs');return false" style="color:var(--primary)">Traffic Ledger</a> page.</p>
            <div class="doc-card">
              <h4>Logged Fields</h4>
              <dl class="doc-dl">
                <dt>request_id</dt><dd>Unique ID for each request</dd>
                <dt>api_key / profile / upstream</dt><dd>Which key, profile, and upstream handled the request</dd>
            ...
            <div class="doc-card mt-4">
              <h4>Filtering</h4>
              <p>Logs are searchable and filterable by profile, upstream, status, and model. Supports sorting by any column and pagination.</p>
            </div>
          </section>
          ...
          <section id="doc-cloudflare">
            ...
            <div class="doc-card">
              ...
              <ol class="doc-ol">
                ...
                <li>In the <a href="#" onclick="OS.navigate('cloudflare');return false" style="color:var(--primary)">Edge Tunnel</a> settings page, enter your API token and click Connect to auto-discover zones and tunnels</li>
          ...
          <section id="doc-cli">
            ...
            <div class="doc-card mt-4">
              <table><thead><tr><th>Command</th><th>Description</th></tr></thead><tbody>
                ...
                <tr><td><code>status</code></td><td>Show upstreams, profiles, keys, and request counts</td></tr>
                <tr><td><code>provider list</code></td><td>List all configured upstreams</td></tr>
                <tr><td><code>provider add</code></td><td>Add a new upstream interactively</td></tr>
                <tr><td><code>provider health</code></td><td>Run health checks on all upstreams</td></tr>
                <tr><td><code>soul list</code></td><td>List all profiles</td></tr>
                <tr><td><code>soul add</code></td><td>Create a new profile interactively</td></tr>
                ...
                <tr><td><code>uninstall</code></td><td>Remove Astra Relay completely</td></tr>
              </tbody></table>
            </div>
          </section>
          ...
          <section id="doc-openclaw">
            ...
            <p>Astra Relay works as a provider backend for OpenClaw agents. Generate a key with auto-configured settings from the console.</p>
            <div class="doc-card">
              <h4>Setup</h4>
              <ol class="doc-ol">
            ...
            <div class="doc-card mt-4">
              <h4>How it works</h4>
              <p>OpenClaw sends requests to Astra Relay using the <code>openai-completions</code> API format. Astra Relay translates the request to the appropriate upstream (Claude, OpenAI, Gemini, etc.) and streams the response back in OpenAI format.</p>
              <p class="form-hint">This means your OpenClaw agents can use Claude, Gemini, or any upstream through a single OpenAI-compatible endpoint — with automatic fallback, rate limiting, and usage logging.</p>
            </div>
            <div class="doc-card mt-4">
              <h4>Multi-account stacking</h4>
              <p>Add multiple accounts of the same upstream type (e.g., 10 Claude subscriptions) and Astra Relay will automatically fall back to the next account if one hits rate limits or goes down. Your OpenClaw agents get uninterrupted access across all accounts.</p>
            </div>
          </section>
          ...
          <section id="doc-sdk">
            <h2>SDK Examples</h2>
            <p>Astra Relay is fully OpenAI-compatible. Point any OpenAI SDK at your gateway URL.</p>
...
# Use a profile endpoint directly
response = client.chat.completions.create(
    model="claude-sonnet-4-6",
    messages=[{"role": "user", "content": "Hello!"}],
    extra_headers={"X-Astra-Profile": "translator"}
)
...
const stream = await client.chat.completions.create({
  ...
  headers: { 'X-Astra-Profile': 'translator' },
});
...
for await (const chunk of stream) {
  process.stdout.write(chunk.choices[0]?.delta?.content || '');
            ...
            <div class="doc-card mt-4">
              <h4>curl</h4>
              <div class="code-block"># Via profile endpoint (recommended)
curl ${host}/v1/souls/translator/chat/completions \\
  -H "Authorization: Bearer ohk_YOUR_KEY" \\
  -H "Content-Type: application/json" \\
  -d '{"messages":[{"role":"user","content":"Hello!"}]}'

# Via generic endpoint with compatibility profile header
curl ${host}/v1/chat/completions \\
  -H "Authorization: Bearer ohk_YOUR_KEY" \\
  -H "X-Astra-Profile: translator" \\
  -H "Content-Type: application/json" \\
  -d '{"model":"gpt-4o","messages":[{"role":"user","content":"Hello!"}]}'</div>
          ...
          <section id="doc-deploy">
            ...
            <div class="doc-card">
              ...
              <p>Run Astra Relay as a background service that starts on login:</p>
...
Description=Astra Relay AI Router
...
docker run -d -p 3700:3700 -v ./data:/app/data -v ./config:/app/config openhinge</div>
  ...
  async function saveProvider(e) {
    ...
    closeModal(); toast('Upstream added', 'success'); loaders.providers();
  }
  ...
  async function editProviderModal(id) {
    ...
    let models = modelLists[p.type] || [];
    try {
      const res = await api(`/admin/providers/${id}/models`);
      if (Array.isArray(res.models) && res.models.length) {
        models = Array.from(new Set([...res.models, ...models]));
      }
    } catch { /* fall back to static model list */ }
  ...
  async function updateProvider(e, id) {
    ...
    closeModal(); toast('Upstream updated', 'success'); loaders.providers();
  }

  async function deleteProvider(id) {
    if (!confirm('Delete this upstream?')) return;
    await api(`/admin/providers/${id}`, { method: 'DELETE' });
    toast('Upstream deleted'); loaders.providers();
  }
      ...
      <form onsubmit="OS.saveSoul(event)" id="soul-form">
        ...
        <div class="form-group">
          ...
          <div id="soul-model-wrapper">
            ...
            <input name="model_manual" id="soul-model-input" placeholder="Select an upstream to load models" class="input-mono">
          </div>
          <div id="soul-model-status" style="margin-top:4px"></div>
        </div>

        <div class="form-group">
          <label class="form-label">Profile Instructions</label>
        ...
        <button type="submit" class="btn btn-primary" style="width:100%;margin-top:8px">Create Profile</button>
      </form>
    `);
  }
  ...
  async function onSoulProviderChange() {
    ...
    if (!providerId) {
      ...
      modelInput.placeholder = 'Select an upstream to load models';
      statusEl.innerHTML = '';
      return;
    }
    ...
    const models = res.models || [];
    if (models.length > 0) {
      modelSelect.innerHTML = '<option value="">Use upstream default</option>' +
  ...
  async function saveSoul(e) {
    ...
    closeModal(); toast('Profile saved', 'success'); loaders.souls();
  }
      ...
      <form onsubmit="OS.saveSoul(event)" id="soul-form">
        ...
        <div class="form-group">
          ...
          <div id="soul-model-wrapper">
            ...
            <input name="model_manual" id="soul-model-input" value="${h(data.model || '')}" placeholder="Select an upstream to load models" class="input-mono">
          </div>
          <div id="soul-model-status" style="margin-top:4px"></div>
        </div>
  ...
  async function deleteSoul(id) {
    ...
    toast('Profile deleted'); loaders.souls();
  }
  ...
  async function createApiKeyForm(format = 'openai') {
    ...
    // Provider info - show which upstream handles requests
    ...
    const soulCheckboxes = _souls.length > 0 ? `
      <div class="form-group">
        ...
        <div style="margin-bottom:6px">
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer;font-size:13px">
            <input type="checkbox" id="key-all-souls" onchange="OS.toggleAllSouls(this.checked)" style="width:auto">
            <strong>All Profiles</strong>
          </label>
        </div>
        <div id="key-soul-list" style="max-height:200px;overflow-y:auto;border:1px solid var(--border);border-radius:var(--radius-sm);padding:8px;display:flex;flex-direction:column;gap:4px">
          ${_souls.map(s => {
        ...
        <p class="form-hint">Select which profiles this key can access. Each profile routes to its own upstream.</p>
      </div>
    ` : '';
    ...
    const routingHint = _souls.length === 0
      ? `<div class="form-group">
          <div style="padding:10px 12px;border:1px solid var(--border);border-radius:var(--radius-sm);background:var(--bg-hover);font-size:12px;color:var(--text-secondary);line-height:1.5">
            <strong style="color:var(--text)">Routes to:</strong> ${providerInfo || 'No upstreams configured'}
            <br>This key will use the default upstream. Create a <strong>Profile</strong> first to control which upstream and model each key uses.
          </div>
        </div>`
      : '';
    ...
    if (soulList) soulList.style.display = 'flex';
    ...
    openModal('Create OpenClaw Key', `
      <form onsubmit="OS.saveOpenClawKey(event)" id="openclaw-key-form">
        ...
        <div class="form-group">
          ...
          <p class="form-hint">The model OpenClaw will request through Astra Relay</p>
        </div>
        <div class="form-group">
          <label class="form-label">Rate Limit (per min)</label>
  ...
  async function disconnectApp(appId, appName) {
    if (!confirm(`Disconnect ${appName} from Astra Relay?`)) return;
  ...
  async function provBulk(action) {
    ...
    if (action === 'delete' && !confirm(`Delete ${ids.length} upstream${ids.length > 1 ? 's' : ''}?`)) return;
    if (action === 'health') {
      toast(`Checking ${ids.length} upstreams...`);
    ...
    if (ok) {
      _provSelected.clear();
      toast(`${action === 'delete' ? 'Deleted' : action === 'enable' ? 'Enabled' : 'Disabled'} ${affected} upstream${affected !== 1 ? 's' : ''}`, 'success');
      loaders.providers();
    }
  }
